...
def diarize_dead_letter_as_meeting(
    *,
    config: AppConfig,
    client: OdinClient,
    job_id: int,
    ...
    try:
        ...
        if job is None:
            return DiarizationResult(
                ...
                items=(DiarizationItem(_missing_job(job_id), "failed_job_not_found", None, None),),
            )
        if job.status != "dead_letter_written" or job.classification != "dead_letter":
            return DiarizationResult(
                diarization_dir=diarization_dir,
                items=(DiarizationItem(job, "failed_not_pending_dead_letter", None, None),),
            )
        item = _diarize_job(job, diarization_dir, client, force_meeting=True)
        if item.status != "diarized" or item.diarization_path is None:
            return DiarizationResult(diarization_dir=diarization_dir, items=(item,))
        result_payload = _read_json(item.diarization_path)
        updated = ledger.rescue_dead_letter_as_meeting_diarized(
...
def _diarize_job(
    ...
    *,
    ...
    if classification.route_kind != "meeting" and not force_meeting:
...
def _missing_job(job_id: int) -> RecordingJob:
    return RecordingJob(
        id=job_id,
        checksum_sha256="",
        source_device="",
