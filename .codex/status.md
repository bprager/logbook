...
Current blocker: the first live meeting recording, `260504_1308.mp3` / job 49, was discovered, copied, transcribed, and classified as `meeting`, but no meeting note has been written to Obsidian because meeting rendering correctly requires diarized speaker labels. The `odin` worker now supports pyannote 4 output and reads the Hugging Face token from its env-file config, but Hugging Face rejected the configured token with 403 gated-repo errors for both `pyannote/speaker-diarization-community-1` and `pyannote/speaker-diarization-3.1`. The next required input is authorizing that token for a pyannote diarization model on Hugging Face, then rerunning job 49 diarization and routing.
