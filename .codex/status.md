...
May 1/2 production batch is processed, dead letters are clear, memory graph drift is repaired, Logbook launchd jobs are installed on `mimir`, and the first `saga` backup/restore drill is complete.
...
- Continue production hardening with LGB-034 release readiness.
